...
class Store {
  constructor(connconf, config) {
    this.connconf = connconf || {}
    this.config = config || {}
  }
  
  setConnectionConfig(connconf) {
    this.connconf = connconf
  }
  
  initConnection() {
    request.setConfig({
      baseURL: `${this.connconf.protocol}://${this.connconf.host}/${this.connconf.db}/${this.connconf.collection}`
    })
  }
  ...
  findOne(query, options) {
    ...
    var result =  {
      ...
      statusText: response.statusText,
      headers: response.headers,
      data: response.data["_embedded"][0] || {} 
    }
    return result
  }
